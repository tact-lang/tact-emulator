import { Account, Address, beginCell, Cell, Message, parseTuple, storeAccount, storeMessage, storeShardAccount, TupleItem, TupleReader } from "ton-core";
...
export class ContractExecutor {
    ...
    async receive(msg: Message) {
        ...
        let res = await this.system.bindings.transaction({
            ...
            libs: null,
            verbosity: 3,
            shardAccount: beginCell().store(storeShardAccount({ account: this.#state, lastTransactionHash: 0n, lastTransactionLt: 0n })).endCell(),
